    ...
    <div className="flex flex-col gap-2 border border-border/60 rounded-2xl px-5 py-4 bg-white/65 dark:bg-zinc-900/40 backdrop-blur shadow-sm hover:shadow transition-all">
      <div className="flex items-center justify-between gap-6">
        ...
        <button
          ...
          onClick={toggle}
          className={`relative inline-flex h-7 w-12 shrink-0 items-center rounded-full transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-primary/40 focus:ring-offset-2 ${on ? 'bg-primary' : 'bg-muted'} ${(pending || remaining.passed) ? 'opacity-50 cursor-not-allowed' : ''}`}
        >
          <span className={`inline-block h-6 w-6 transform rounded-full bg-white dark:bg-zinc-100 shadow ring-1 ring-black/5 dark:ring-white/10 transition-transform duration-300 ${on ? 'translate-x-6' : 'translate-x-0.5'}`}/>
        </button>
      </div>
