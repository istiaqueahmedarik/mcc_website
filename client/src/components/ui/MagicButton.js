...
const MagicButton = ({
  ...
  otherClasses,
  textClass,
  bgColor = "bg-slate-950 dark:bg-slate-950",
}) => {
  return (
    <button
      onClick={handleClick}
    ...
    >
      ...
      >
        ...
        <span className={`relative z-10 flex items-center gap-2 text-white dark:text-white group-hover:text-slate-950 dark:group-hover:text-white font-medium transition-colors duration-500 ${textClass}`}>
          {position === "left" && icon}
          {title}
          {position === "right" && icon}
        </span>
