...
import { Calendar, Clock, Search, Filter, Link2, AlertTriangle, Database, RefreshCw, CheckCircle } from 'lucide-react';
import { UnifiedContest } from '@/lib/data-sources/unified';
import { saveContestsList, deleteContestsList } from '@/actions/contest';
...
export default function ContestsClient({ 
  ...
  const handleFetchUrl = (e: React.FormEvent) => {
    ...
    if (parsed) {
      router.push(`/contests/${parsed.provider}/${parsed.slug}`);
    } else {
