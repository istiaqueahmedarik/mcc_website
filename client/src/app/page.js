  ...
  return (
    <main className="scroll-smooth w-full flex flex-col gap-32 justify-center items-center overflow-x-hidden bg-background">
      ...
      <section className="relative min-h-[90vh] w-full flex justify-center items-center px-4 pt-20">
        <div className="flex justify-center w-full items-center flex-col max-w-3xl gap-8 text-center">
          <TextGenerateEffect
            className="uppercase text-3xl md:text-5xl font-bold"
            words="Hello Programmer, Welcome To"
          />
          <MccLogo classes="animate-appear drop-shadow-xl" w={260} h={260} />
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full items-center sm:w-auto">
            <MagicButton
              title="Join Now"
              position="right"
              ...
              bgColor="dark:bg-slate-950 bg-white"
              textClass="group-hover:text-primary-foreground"
            />
            ...
            <MagicButton
              ...
              textClass="group-hover:text-secondary-foreground"
            />
          </div>
        </div>
      ...
      <section ref={exploreRef} className="w-full max-w-7xl px-4">
        ...
        <div className="grid md:grid-cols-4 sm:grid-cols-2 grid-cols-1 gap-6 mb-16">
          {features.map((f) => (
            <div
              key={f.title}
              className="group relative rounded-xl border border-border/60 p-[1px] bg-gradient-to-br from-primary/10 via-background to-background hover:from-primary/30 transition-colors overflow-hidden"
            >
              <div
                className="pointer-events-none absolute inset-0 opacity-0 group-hover:opacity-100 transition duration-300"
                style={{
                  background: `radial-gradient(600px circle at var(--x) var(--y), rgba(250,204,21,0.35), transparent 40%)`,
                }}
              />
              <div
                onMouseMove={(e) => {
                ...
                className="h-full rounded-[inherit] p-5 flex flex-col justify-between bg-background/80 backdrop-blur relative z-10"
              >
                <div>
                  <h3 className="font-semibold text-sm md:text-base tracking-wide mb-2 group-hover:text-yellow-400 transition-colors">
                    {f.title}
                  </h3>
