...
export default async function Page({ searchParams }) {
  ...
  const visibleCollections =
    activeTab === "finalized" ? finalizedCollections : collectingCollections;
  ...
  await Promise.all(
    latestCollectionIds.map(async (collectionId) => {
      try {
                              ...
                              <div className="flex flex-col gap-3">
                                ...
                                >
                                  ...
                                  <button className="px-4 py-2 rounded-md bg-accent hover:bg-accent/30 text-foreground text-xs font-semibold flex items-center gap-1 w-fit">
                                    <FastForward className="w-4 h-4" /> Start
                                    Phase 2 Now
                                  </button>
                                </TeamActionForm>
                              </div>
                ...
                <ScrollArea className="w-full rounded-md border">
                  <div className="grid gap-4 p-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4">
                    {rooms.map((r) => {
                      ...
                      const totalContests =
                        contestsFromMap ?? contestsFromRoom ?? null;
