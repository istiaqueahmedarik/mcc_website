...
export default function VJudgeVerifyClient(){
  ...
  const load = async () => {
    ...
    setError("")
    try {
      const res = await get_with_token('user/vjudge/pending')
  ...
  const setVerify = async (user_id, verified=true) => {
    try {
      await post_with_token('user/vjudge/verify', { user_id, verified })
      await load()
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      <Card>
        <CardHeader>
          <CardTitle>VJudge Verification (Admin)</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            {rows.map(u => (
              <div key={u.id} className="flex items-center justify-between border rounded-md p-3">
                <div>
                  ...
                  <div className="text-sm">VJudge: <span className="font-mono">{u.vjudge_id}</span></div>
                  <div className="text-sm">Codeforces: <span className="font-mono">{u.cf_id || '—'}</span> {u.cf_verified && <Badge className="ml-2">CF Verified</Badge>}</div>
                </div>
