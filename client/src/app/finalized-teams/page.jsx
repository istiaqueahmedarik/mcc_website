    ...
    <main className="min-h-screen bg-gradient-to-br from-background via-muted/20 to-background py-12">
      <div className="max-w-7xl mx-auto px-5 space-y-16">
        <header className="text-center space-y-2">
          <h1 className="text-4xl md:text-5xl font-semibold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-foreground to-foreground/60">
            Finalized Teams
          </h1>
          {isAdmin && (
            <p className="inline-flex items-center gap-2 text-xs bg-primary/10 text-primary px-3 py-1 rounded-full border border-primary/20">
              Admin View Enabled
            </p>
          )}
        </header>
        ...
        {blocks.map((block) => (
          <section
            ...
            className="space-y-6"
            aria-labelledby={`c-${block.collection_id}`}
          >
            <div className="flex items-center justify-between text-center flex-wrap gap-4">
              <h2
                id={`c-${block.collection_id}`}
                className="text-2xl font-semibold text-center tracking-tight flex items-center gap-3"
              >
                <span>{block.collection_title || "Untitled Collection"}</span>
                <span className="inline-flex h-6 w-32 items-center justify-center rounded-lg bg-gradient-to-br from-primary/30 to-primary/10 text-primary/60 shadow-inner">
                  {block.teams.length} teams
                </span>
              </h2>
            </div>
            ...
            {!isAdmin && (
              <>
                ...
                <div className="px-2 pb-2 text-xs text-center font-medium uppercase tracking-wider text-muted-foreground">
                  Public Leaderboard
                </div>
                          ...
                          return (
                            ...
                            >
                              ...
                              <TableCell className="text-muted-foreground/90">
                                {profileMap.get(m)?.mist_id || "-"}
                              </TableCell>
                              <TableCell>
                                {deriveLevel(profileMap.get(m)?.mist_id)}
                              </TableCell>
                            </TableRow>
                          );
                        });
                                  ...
                                  {t.coach_vjudge_id && (
                                    ...
                                    >
                                      ...
                                      <div className="flex flex-col">
                                        ...
                                        <span
                                          rowSpan={t.members.length}
                                          className="text-muted-foreground/90 text-[12px]"
                                        >
                                          {coachProfile.phone || "-"}
                                        </span>
