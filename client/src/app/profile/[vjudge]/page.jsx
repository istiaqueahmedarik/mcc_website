...
function deriveUserPerformanceAcrossReports(vjudge, reports) {
  ...
  const bestRank = unique.reduce(
    (best, r) => (r.rank && (!best || r.rank < best) ? r.rank : best),
    undefined
  );
  const avgRank =
    unique.length > 0
      ? unique.reduce((s, r) => s + (r.rank || 0), 0) /
          unique.filter((r) => r.rank).length || 0
      : 0;
  return {
    rows: unique,
    summary: {
...
export default async function PublicProfilePage({ params }) {
          ...
          <Card className="apple-card max-w-md mx-auto text-center">
            <CardContent className="pt-12 pb-8">
              ...
              <p className="text-muted-foreground text-pretty">
                No user linked to VJudge ID: {vjudge}
              </p>
            </CardContent>
          </Card>
        </div>
  ...
  async function loadTeams() {
    try {
      const base = process.env.NEXT_PUBLIC_SERVER_URL || process.env.SERVER_URL;
      const res = await fetch(
        `${base}/team-collection/public/teams/by-vjudge`,
        {
          method: "POST",
          cache: "no-store",
  ...
  async function loadCoachedTeams() {
    try {
      const base = process.env.NEXT_PUBLIC_SERVER_URL || process.env.SERVER_URL;
      const res = await fetch(
        `${base}/team-collection/public/teams/coached-by-vjudge`,
        {
          method: "POST",
          cache: "no-store",
          headers: { "Content-Type": "application/json" },
  ...
  const perf = deriveUserPerformanceAcrossReports(
    vjudge,
    allReports?.result || []
  );
            ...
            <div className="profile-card profile-sidebar-sticky">
              <div className="flex flex-col items-center text-center space-y-4">
                ...
                <div className="relative group">
                  <div
                    className="w-32 h-32 rounded-2xl overflow-hidden border-2 transition-all duration-300 hover:scale-105 hover:shadow-xl"
                ...
                <div className="w-full">
                  <h1
                    className="text-2xl font-bold mb-3 break-words transition-colors duration-200"
                  ...
                  <div className="flex flex-wrap gap-2 justify-center">
                    {u.vjudge_verified && (
                      <span
                        className="profile-badge text-xs transition-all duration-200 hover:scale-105 hover:shadow-md"
                        style={{
                    ...
                    {u.cf_verified && (
                      <span
                        className="profile-badge text-xs transition-all duration-200 hover:scale-105 hover:shadow-md"
                        style={{
                          background: "hsl(var(--profile-danger))",
                          color: "white",
                        }}
                      >
                        CF Verified
                ...
                >
                  <div className="flex items-center gap-2 justify-center transition-all duration-200 hover:translate-x-1">
                    <div
                      ...
                      style={{ background: "hsl(var(--profile-primary))" }}
                    ></div>
                    <span>
                      Joined{" "}
                  ...
                  {u.email && (
                    <div className="flex items-center gap-2 justify-center break-all transition-all duration-200 hover:translate-x-1">
                      <div
                        ...
                        style={{ background: "hsl(var(--profile-success))" }}
                      ></div>
                  ...
                  {u.phone && (
                    <div className="flex items-center gap-2 justify-center transition-all duration-200 hover:translate-x-1">
                      <div
                        className="w-1.5 h-1.5 rounded-full"
                        style={{ background: "hsl(var(--profile-warning))" }}
                      ></div>
                      <span>{u.phone}</span>
                    </div>
                  )}
              ...
              <div className="profile-card mb-6">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  {teams.map((t) => (
                    ...
                    >
                      <div
                        className="font-bold text-lg mb-2 text-balance transition-colors duration-200 group-hover:underline"
                        style={{ color: "hsl(var(--profile-text))" }}
                      >
                        {t.team_title}
                      </div>
                  ...
                  {coachedTeams.map((t) => (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-2">
                        <span
                          className="profile-badge text-xs transition-all duration-200 hover:scale-105"
                          style={{
                            background: "hsl(var(--profile-warning) / 0.2)",
                            color: "hsl(var(--profile-warning))",
                          }}
                        >
                          Coach
              ...
              <div className="space-y-6">
                ...
                <Tabs defaultValue="graph" className="w-full">
                  <TabsList
                    ...
                    style={{ background: "hsl(var(--profile-surface-2))" }}
                  >
                    <TabsTrigger
                      value="table"
                ...
                >
                  <CodeforcesSubmissionDashboard
                    users={[
                      {
                        id: u.id,
                        full_name: u.full_name || u.vjudge_id,
                        cf_id: u.cf_id,
                      },
                    ]}
                  />
