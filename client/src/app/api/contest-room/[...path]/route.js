...
function buildBackendPath(params) {
...
async function forward(request, context, method) {
  ...
  const targetUrl = `${serverBase}/${buildBackendPath(params)}${new URL(request.url).search}`;
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to reach contest-room service" }, { status: 500 });
  }
}
