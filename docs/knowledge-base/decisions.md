...
Source:
- `docs/rsd/classroom-codeforces-api-first-fallback-20260902-rsd.md`
- `server/src/services/codeforcesContestService.ts`
...
Global Codeforces report configuration records an explicit Public, Gym, Group, or EDU source type. Public/Gym/EDU participants resolve through the Codeforces handle saved on an eligible MCC account. Group participants use either the numeric suffix after the last `=` or a replaceable `username,student_id` CSV. VJudge resolves through the saved VJudge ID. Any external alias must resolve to exactly one immutable `users.id` before shared scoring; missing, duplicate, ambiguous, or multi-student matches are omitted from the report. The uploaded CSV is never retained. Source: `docs/rsd/global-codeforces-identity-resolution-20260919-rsd.md`.

Global generated contest reports use private database snapshots as the normal read path. First generation and explicit Refresh may contact providers; every other report view loads the saved scope, and Publish copies the current non-stale full-room snapshot without provider traffic. Authoritative room/configuration mutations mark snapshots stale instead of deleting the last reviewable report. Source: `docs/rsd/global-contest-report-snapshot-cache-20260919-rsd.md`.
