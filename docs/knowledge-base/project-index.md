...
The approved classroom interior implementation is in `client/src/app/classroom/live/[id]/ClassroomLiveClient.js`, `TrainerClassroomInterior.jsx`, `trainer-interior-model.mjs`, and `client/src/components/ClassroomContestPanel.jsx`. Trainers retain visible Updates, Live, Topics, People, and Contests destinations; students retain their existing navigation. Updates leads with real next/live session context and separate pending account/submission attention. Live uses a focused assignment dialog plus a right-side selected-work inspector on desktop and a focus-managed dialog on mobile. The approved Contests v2 revision restores a bounded 280px room rail beside the report workspace, stacks it above the workspace on mobile, and adds client-only room sorting through `classroom-contest-room-sort.mjs`; Name A-Z is the default. Existing handlers, permissions, report/scoring behavior, and provider-session safety remain authoritative. Review: `docs/reviews/classroom-interior-design-20260905-implementation-review.md`; editable design page: `05 · Contests v2 - review` in `/home/arik/Documents/Classroom Interior Design.tldraw`.
...
Fact:
Classroom-private Codeforces reports accept EDU lesson standings URLs in addition to numeric contests. Plain URLs and legacy `edu:<course>:<lesson>` identities fetch the authenticated `friends=true` view with a trainer-provided HTTP-only JSESSIONID; explicit `list=<readKey>` sources remain supported. The adapter parses solved and rejected-attempt penalty separately, persists only verified/overridden classroom handles, and fails with actionable guidance when no classroom student appears in the trainer's friends standings. The crawler has bounded response size, concurrency, and page count; missing/expired sessions fail before snapshot persistence. The Supabase identifier-constraint migration was applied and verified on 2026-09-01. The classroom live shell caps at 1600px, and dense report tables expose labelled, keyboard-focusable horizontal scrolling.
...
Do not overgeneralize:
This does not add EDU support to global legacy reports, store Codeforces web sessions in Postgres, mutate Codeforces lists/friends, accept arbitrary crawl origins, or auto-refresh standings.
