...
export const adminGetCollectionDetail = async (c: any) => {
    ...
    try {
        ...
        const parts = await sql`SELECT vjudge_id FROM public.team_collection_participation WHERE collection_id=${collection_id} AND will_participate=true AND vjudge_id IS NOT NULL`
        if (parts.length > 0) {
...
export const adminStartPhase2 = async (c: any) => {
    ...
    try {
        ...
        const clientUrl = process.env.NEXT_PUBLIC_CLIENT_URL || 'https://mistcomputerclub.vercel.app'
