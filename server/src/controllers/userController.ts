...
export const setCodeforcesId = async (c: Ctx) => {
  ...
  try {
    ...
    const rows =
      await sql`update users set cf_id = ${cf_id}, cf_verified = false where id = ${id} returning id, cf_id, cf_verified`;
...
export const listCodeforcesPending = async (c: Ctx) => {
  ...
  try {
    const rows =
      await sql`select id, full_name, email, cf_id, cf_verified, vjudge_id, vjudge_verified from users where cf_id is not null and cf_id <> '' and cf_verified = false order by created_at desc`;
...
export const verifyCodeforces = async (c: Ctx) => {
  ...
  try {
    ...
    if (!flag) {
      rows =
        await sql`update users set cf_id = null, cf_verified = false where id = ${user_id} returning id, cf_id, cf_verified`;
    } else {
      rows =
        await sql`update users set cf_verified = ${flag} where id = ${user_id} returning id, cf_id, cf_verified`;
    }
...
export const setVjudgeId = async (c: any) => {
  ...
  try {
    ...
    const rows =
      await sql`update users set vjudge_id = ${vjudge_id}, vjudge_verified = false where id = ${id} returning id, vjudge_id, vjudge_verified`;
    return c.json({ result: rows[0] });
  } catch (e) {
    console.error(e);
...
export const listVjudgePending = async (c: any) => {
  ...
  try {
    const rows =
      await sql`select id, full_name, email, vjudge_id, vjudge_verified, cf_id, cf_verified from users where vjudge_id is not null and vjudge_id <> '' and vjudge_verified = false order by created_at desc`;
    return c.json({ result: rows });
  } catch (e) {
    console.error(e);
    return c.json({ error: "Failed to load pending list" }, 400);
...
export const setTshirtSize = async (c: any) => {
  ...
  try {
    ...
    const rows =
      await sql`update users set tshirt_size = ${tshirt_size} where id = ${id} returning id, tshirt_size`;
    if (rows.length === 0) return c.json({ error: "User not found" }, 404);
    return c.json({ result: rows[0] });
  } catch (e) {
    console.error(e);
...
export const setProfilePic = async (c: any) => {
  ...
  try {
    ...
    const rows =
      await sql`update users set profile_pic = ${profile_pic} where id = ${id} returning id, profile_pic`;
    if (rows.length === 0) return c.json({ error: "User not found" }, 404);
    return c.json({ result: rows[0] });
  } catch (e) {
    console.error(e);
